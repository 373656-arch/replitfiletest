...
// 2.5 INTERCEPT AI ASSISTANT AJAX REQUESTS (JSON PAYLOADS)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && strpos($_SERVER['CONTENT_TYPE'] ?? '', 'application/json') !== false) {
    header('Content-Type: application/json');

    // PHP can't read JSON from $_POST, so we read the raw input stream directly
    $rawInput = file_get_contents('php://input');
    $inputData = json_decode($rawInput, true);

    if (!$inputData || empty($inputData['message'])) {
    ...
    $availableParts = $inputData['available_parts'] ?? [];
    $currentBuild   = $inputData['current_build'] ?? [];

    /* -----------------------------------------------------------------
      YOUR AI / LLM API LOGIC GOES HERE
      -----------------------------------------------------------------
      When you're ready, this is where you'll use cURL to send $userMessage
      to Gemini, OpenAI, etc., alongside your array of $availableParts.

      For now, let's set up a test response structure so your frontend 
      doesn't crash and you can see it working in real-time.
    */

    $aiResponseText = "I heard you! You said: \"" . $userMessage . "\". Once our LLM connection is live, I'll recommend custom parts here.";
    $actions = [];

    // Simple keyword test to prove the frontend tags/actions work:
    if (stripos($userMessage, 'exhaust') !== false) {
        // Find the first compatible exhaust part to add automatically
        foreach ($availableParts as $part) {
            if ($part['category'] === 'Exhaust' && $part['compatible']) {
                $actions[] = [
                    'action'  => 'add',
                    'part_id' => $part['id']
                ];
                $aiResponseText = "I found a compatible exhaust! Adding the " . $part['name'] . " to your setup.";
                break;
            }
        }
    } elseif (stripos($userMessage, 'clear') !== false || stripos($userMessage, 'fresh') !== false) {
        // Build 'remove' actions for every single item currently on the canvas
        foreach ($currentBuild as $part) {
            $actions[] = [
                'action'  => 'remove',
                'part_id' => $part['part_id']
            ];
        }
        $aiResponseText = "Alright, I've cleared out your build sheet. Let's start from scratch!";
    }

    // Return the exact keys your JavaScript 'sendAIMessage()' is looking for
    echo json_encode([
        'message' => $aiResponseText,
        'actions' => $actions
    ]);

    exit; // CRITICAL: Stop script execution here so no HTML is outputted!
}
